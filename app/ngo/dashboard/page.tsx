...
import { getAvailableDonations, getClaimsByNGO, getUserById, type FoodDonation, type Claim } from "@/lib/store"
...
export default function NGODashboard() {
  ...
  const [claimedDonations, setClaimedDonations] = useState<Claim[]>([])
  ...
  useEffect(() => {
    ...
    if (userData) {
      setUser(userData)
      refreshData(currentUser.userId)
    }
    setLoading(false)
  }, [router])
  ...
  const refreshData = (userId: string) => {
    ...
    const claimed = getClaimsByNGO(userId)
