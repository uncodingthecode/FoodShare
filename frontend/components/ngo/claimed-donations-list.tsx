...
export default function ClaimedDonationsList({ claims, onStatusUpdate }: ClaimedDonationsListProps) {
  ...
  const handleMarkCompleted = async (claimId: string) => {
    if (!confirm('Mark this claim as completed?')) {
      return
    }

    setUpdatingId(claimId)
    try {
      await claimsAPI.updateStatus(claimId, "completed")
  ...
  return (
    <div className="space-y-4">
      ...
      {claims.map((claim) => {
        ...
        return (
          <div key={claim._id} className="card border-l-4 border-l-primary">
            ...
            {claim.status === "pending" && (
              <div className="flex gap-2 pt-4 border-t border-border">
                <button
                  onClick={() => handleMarkCompleted(claim._id)}
